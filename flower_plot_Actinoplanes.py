# -*- coding: utf-8 -*-
"""
Created on Sat Dec 12 09:46:04 2020
...
#Read data
my_data = pd.read_table('stats_actinoplanes.txt',  sep='\t')
...
ax.pie(petals, radius=1.3, colors=["lime"],
       labels = organisms, 
...
ax.pie(petals, radius=0.9, colors=["coral"],
       labels = list(unique_genes), 
        labeldistance=1,  rotatelabels = 45,
       wedgeprops=dict(width=0.6, edgecolor='w'), 
       textprops=dict(fontsize=8))

ax.pie(core_genes, radius=0.5, colors=['skyblue'], 
       labels = list(accessory_genes),
       rotatelabels = 45, labeldistance=0.92,
       wedgeprops=dict(width=1), textprops=dict(fontsize=8))

ax.text(-0.45, 0, 'Core genome ', fontsize = 9)
ax.text(-0.2, -0.2, '{0:0.0f}'.format(core_genes[0]), fontsize = 8)
ax.set(aspect="equal")


fig.savefig("flower_plot_Actinoplanes2", dpi = 1200, format = 'svg')
